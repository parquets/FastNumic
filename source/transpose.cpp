...
static inline void transpose_block_16X16(const float* A, int lda, 
                                         float* B, int ldb) {
...
static inline void transpose_block_32X16(const float* A, int lda,
                                         float* B, int ldb) {
    transpose_block_16X16(A, lda, B, ldb);
    transpose_block_16X16(A+16*lda, lda, B+16, ldb);
}

static inline void transpose_block_32X32(const float* A, int lda,
                                         float* B, int ldb) {
    transpose_block_16X16(A, lda, B, ldb);
...
static inline void transpose_block_16X32(const float* A, int lda,
                                         float* B, int ldb) {
    transpose_block_16X16(A, lda, B, ldb);
    transpose_block_16X16(A+16, lda, B+16*ldb, ldb);
}
...
void transpose(int M, int N, const float* A, int lda, float* B, int ldb) {
    ...
    for (r = 0; r < M - 31; r += 32) {
        ...
        if (c < N) kernel::transpose_common(A + r * lda + c, 32, N - c, lda, B + c * ldb + r, ldb);
    }
    ...
    for (; r < M - 15; r += 16) {
        ...
        if (c < N) kernel::transpose_common(A + r * lda + c, 16, N - c, lda, B + c * ldb + r, ldb);
    }
    if (r < M) kernel::transpose_common(A + r * lda, M - r, N, lda, B + r, ldb); 
}
