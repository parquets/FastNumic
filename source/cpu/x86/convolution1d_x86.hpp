...
static inline void conv1dWeightPackOCUnit8(float* dest, const float* weight,
    ...
    for(; ic < in_channels; ++ic) {
        for(int k = 0; k < kernel_size; ++k) {
            dest[0] = weight_ptr0[k];
            dest[1] = weight_ptr1[k];
            dest[2] = weight_ptr2[k];
            dest[3] = weight_ptr3[k];
            dest[4] = weight_ptr4[k];
            dest[5] = weight_ptr5[k];
            dest[6] = weight_ptr6[k];
            dest[7] = weight_ptr7[k];
            dest += 8;
        }
...
static inline void conv1dWeightPackOCUnit1(float* dest, const float* weight,
                                           int in_channels, int out_channels,
                                           int kernel_size) {
    const float* weight_ptr0 = weight + 0 * in_channels * kernel_size;
    __m256i _v_index = _mm256_setr_epi32(0, 1, 2, 3, 4, 5, 6, 7);
...
static inline void conv1dOCUnit4(float* dest, const float* source, 
                                 const float* packed_weight,
                                 int in_channels, int out_channels,
                                 int in_w, int kernel_w, int stride_w,
                                 int dilation_w) {
    int out_w = (in_w - kernel_w) / stride_w + 1;
    for(int w=0; w < out_w; ++w) {
        __m128 _v_sum0 = _mm_setzero_ps();
        __m128 _v_sum1 = _mm_setzero_ps();
        __m128 _v_sum2 = _mm_setzero_ps();
        __m128 _v_sum3 = _mm_setzero_ps();

        int ic = 0;
        float* dest_ptr = dest + w;
        for(ic = 0; ic < in_channels - 7; ic += 8) {
            const float* source_ptr = source + ic * in_w + w * stride_w;
            const float* pkw_ptr    = packed_weight + ic * 4 * kernel_w;
            for(int k=0; k < kernel_w; ++k) {
                __m128 _v_r0 = _mm_set1_ps(source_ptr[0 * in_w]);
                __m128 _v_r1 = _mm_set1_ps(source_ptr[1 * in_w]);
                __m128 _v_r2 = _mm_set1_ps(source_ptr[2 * in_w]);
                __m128 _v_r3 = _mm_set1_ps(source_ptr[3 * in_w]); 
                __m128 _v_r4 = _mm_set1_ps(source_ptr[4 * in_w]);
                __m128 _v_r5 = _mm_set1_ps(source_ptr[5 * in_w]);
                __m128 _v_r6 = _mm_set1_ps(source_ptr[6 * in_w]);
                __m128 _v_r7 = _mm_set1_ps(source_ptr[7 * in_w]);

                __m128 _v_w0 = _mm_loadu_ps(pkw_ptr + 0 * 4);
                __m128 _v_w1 = _mm_loadu_ps(pkw_ptr + 1 * 4);
                __m128 _v_w2 = _mm_loadu_ps(pkw_ptr + 2 * 4);
                __m128 _v_w3 = _mm_loadu_ps(pkw_ptr + 3 * 4);
                __m128 _v_w4 = _mm_loadu_ps(pkw_ptr + 4 * 4);
                __m128 _v_w5 = _mm_loadu_ps(pkw_ptr + 5 * 4);
                __m128 _v_w6 = _mm_loadu_ps(pkw_ptr + 6 * 4);
                __m128 _v_w7 = _mm_loadu_ps(pkw_ptr + 7 * 4);

                _v_sum0 = _mm_fmadd_ps(_v_r0, _v_w0, _v_sum0);
                _v_sum1 = _mm_fmadd_ps(_v_r1, _v_w1, _v_sum1);
                _v_sum2 = _mm_fmadd_ps(_v_r2, _v_w2, _v_sum2);
                _v_sum3 = _mm_fmadd_ps(_v_r3, _v_w3, _v_sum3);
                _v_sum0 = _mm_fmadd_ps(_v_r4, _v_w4, _v_sum0);
                _v_sum1 = _mm_fmadd_ps(_v_r5, _v_w5, _v_sum1);
                _v_sum2 = _mm_fmadd_ps(_v_r6, _v_w6, _v_sum2);
                _v_sum3 = _mm_fmadd_ps(_v_r7, _v_w7, _v_sum3);
        ...
        _v_sum0 = _mm_add_ps(_v_sum0, _v_sum1);
        _v_sum2 = _mm_add_ps(_v_sum2, _v_sum3);
        _v_sum0 = _mm_add_ps(_v_sum0, _v_sum2);

        float sum[4];
        _mm_storeu_ps(sum, _v_sum0);
...
static inline void conv1dOCUnit1(float* dest, const float* source, 
                                 const float* packed_weight,
                                 int in_channels, int out_channels,
                                 int in_w, int kernel_w, int stride_w,
                                 int dilation_w) {
    ...
    for(int w=0; w < out_w; ++w) {
        ...
        for(ic = 0; ic < in_channels - 7; ic += 8) {
            const float* source_ptr = source + ic * in_w + w * stride_w;
            const float* pkw_ptr    = packed_weight + ic * 1 * kernel_w;

            for(int k=0; k < kernel_w; ++k) {
                sum0 += source_ptr[0 * in_w] * pkw_ptr[0];
                sum1 += source_ptr[1 * in_w] * pkw_ptr[1];
                sum2 += source_ptr[2 * in_w] * pkw_ptr[2];
                sum3 += source_ptr[3 * in_w] * pkw_ptr[3];
                sum0 += source_ptr[4 * in_w] * pkw_ptr[4];
                sum1 += source_ptr[5 * in_w] * pkw_ptr[5];
                sum2 += source_ptr[6 * in_w] * pkw_ptr[6];
                sum3 += source_ptr[7 * in_w] * pkw_ptr[7];
