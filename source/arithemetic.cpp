...
void add_scaler(int M, int N, const float* A, int lda, const float B, float* C, int ldc) {
    ...
    for (; m < M - 3; m += 4) {
        kernel::add_scaler(C + (m + 0) * ldc, A + (m + 0) * lda, B + (m + 0) * ldb, N);
        kernel::add_scaler(C + (m + 1) * ldc, A + (m + 1) * lda, B + (m + 1) * ldb, N);
        kernel::add_scaler(C + (m + 2) * ldc, A + (m + 2) * lda, B + (m + 2) * ldb, N);
        kernel::add_scaler(C + (m + 3) * ldc, A + (m + 3) * lda, B + (m + 3) * ldb, N);
    }
    for(; m<M; ++m) {
        kernel::add_scaler(C + (m + 0) * ldc, A + (m + 0) * lda, B + (m + 0) * ldb, N);
    }
...
void sub_scaler(int M, int N, const float* A, int lda, const float B, float* C, int ldc) {
    ...
    for (; m < M - 3; m += 4) {
        kernel::sub_scaler(C + (m + 0) * ldc, A + (m + 0) * lda, B + (m + 0) * ldb, N);
        kernel::sub_scaler(C + (m + 1) * ldc, A + (m + 1) * lda, B + (m + 1) * ldb, N);
        kernel::sub_scaler(C + (m + 2) * ldc, A + (m + 2) * lda, B + (m + 2) * ldb, N);
        kernel::sub_scaler(C + (m + 3) * ldc, A + (m + 3) * lda, B + (m + 3) * ldb, N);
    }
    for(; m<M; ++m) {
        kernel::sub_scaler(C + (m + 0) * ldc, A + (m + 0) * lda, B + (m + 0) * ldb, N);
    }
...
void mul_scaler(int M, int N, const float* A, int lda, const float B, float* C, int ldc) {
    ...
    for (; m < M - 7; m += 8) {
        ...
        kernel::mul_scaler(C + (m + 4) * ldc, A + (m + 4) * lda, B, N);
        kernel::mul_scaler(C + (m + 5) * ldc, A + (m + 5) * lda, B, N);
        kernel::mul_scaler(C + (m + 6) * ldc, A + (m + 6) * lda, B, N);
        kernel::mul_scaler(C + (m + 7) * ldc, A + (m + 7) * lda, B, N);
    }
    for (; m < M - 3; m += 4) {
        kernel::mul_scaler(C + (m + 0) * ldc, A + (m + 0) * lda, B, N);
        kernel::mul_scaler(C + (m + 1) * ldc, A + (m + 1) * lda, B, N);
